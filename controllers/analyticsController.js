...
export const getDashboardOverview = async (req, res, next) => {
  try {
    ...
    const recentOrders = await Order.find()
      ...
      .limit(5)
      .select(
        "orderId customer.name total orderStatus paymentStatus createdAt"
      );
    ...
    const dashboard = {
      revenue: {
        ...
        averageOrderValue: revenueData[0]?.totalRevenue
          ? Math.round(
              (revenueData[0].totalRevenue / revenueData[0].totalOrders) * 100
            ) / 100
          : 0,
      },
      orders: {
  ...
  try {
    ...
    return ResponseHandler.success(res, 200, getMessage("SUCCESS", language), {
      summary: {
        ...
        averageOrderValue:
          Math.round((totalRevenue[0]?.average || 0) * 100) / 100,
      },
      revenueOverTime,
...
export const exportAnalyticsReport = async (req, res, next) => {
  try {
    ...
    // JSON format
    return ResponseHandler.success(res, 200, "Analytics report generated", {
      reportDate: new Date(),
