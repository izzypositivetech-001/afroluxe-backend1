...
export const orderConfirmationTemplate = (order, language = 'en') => {
  ...
  const itemsHtml = order.items.map(item => `
    <div class="order-item">
      ...
      ${t.quantity}: ${item.quantity} × ${item.price.toFixed(2)} NOK = ${(item.quantity * item.price).toFixed(2)} NOK
    </div>
  `).join('');
  ...
  const content = `
    ...
    <div class="info-box">
      ...
      <p><strong>${t.orderDate}:</strong> ${new Date(order.createdAt).toLocaleDateString(language === 'en' ? 'en-US' : 'nb-NO')}</p>
    </div>
    ...
    <div class="order-details">
      ...
      <table style="margin-top: 15px;">
        ...
        <tr>
          ...
          <td class="total" style="text-align: right;">${order.total.toFixed(2)} NOK</td>
        </tr>
      </table>
    </div>
...
export const shippingNotificationTemplate = (order, language = 'en') => {
  ...
  const content = `
    ...
    <div class="success-box">
      <h4>${t.tracking}</h4>
      <p><strong>${t.trackingNumber}:</strong> ${order.trackingNumber || 'N/A'}</p>
      <p><strong>${t.carrier}:</strong> ${order.carrier || 'N/A'}</p>
      ${order.estimatedDelivery ? `<p><strong>${t.estimatedDelivery}:</strong> ${new Date(order.estimatedDelivery).toLocaleDateString(language === 'en' ? 'en-US' : 'nb-NO')}</p>` : ''}
    </div>
...
export const paymentConfirmationTemplate = (order, language = 'en') => {
  ...
  const content = `
    ...
    <div class="success-box">
      ...
      <p><strong>${t.date}:</strong> ${new Date().toLocaleDateString(language === 'en' ? 'en-US' : 'nb-NO')}</p>
    </div>
...
export const orderCancellationTemplate = (order, language = 'en') => {
  const translations = {
    en: {
      ...
      refundText: 'If you have already paid, a refund will be processed within 5-7 business days.',
      button: 'View Order'
    },
    no: {
      ...
      refundText: 'Hvis du allerede har betalt, vil refusjon behandles innen 5-7 virkedager.',
      button: 'Vis bestilling'
    }
  };
...
export const adminNotificationTemplate = (type, data, language = 'en') => {
  ...
  return {
    subject: type === 'new_order' ? t.newOrder : type === 'low_stock' ? t.lowStock : t.orderCancelled,
    html: emailWrapper(content, language)
  };
};