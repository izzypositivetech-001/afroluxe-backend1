...
export const sendOrderConfirmation = async (order, language = 'en') => {
  try {
    ...
    const shippingInfo = language === 'en'
      ? 'Shipping Address:'
      : 'Leveringsadresse:';
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>${language === 'en' 
          ...
          : 'Din bestilling er mottatt og behandles.'}</p>
        
        <h3>${orderDetails}</h3>
        <p><strong>${language === 'en' ? 'Order ID' : 'Bestillings-ID'}:</strong> ${order.orderId}</p>
        <p><strong>${language === 'en' ? 'Order Date' : 'Bestillingsdato'}:</strong> ${new Date(order.createdAt).toLocaleDateString()}</p>
        
        <h4>${language === 'en' ? 'Items:' : 'Varer:'}</h4>
        <pre>${itemsList}</pre>
        
        <div style="margin-top: 20px; padding: 10px; background-color: #f5f5f5;">
          <p><strong>${language === 'en' ? 'Subtotal' : 'Delsum'}:</strong> ${order.subtotal} NOK</p>
          <p><strong>${language === 'en' ? 'Tax (25% MVA)' : 'Skatt (25% MVA)'}:</strong> ${order.tax} NOK</p>
          <p><strong>${language === 'en' ? 'Shipping' : 'Frakt'}:</strong> ${order.shippingFee} NOK</p>
          <p style="font-size: 18px;"><strong>${language === 'en' ? 'Total' : 'Totalt'}:</strong> ${order.total} NOK</p>
        </div>
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>${language === 'en' 
          ...
          : `Din bestilling ${order.orderId} status er oppdatert til: ${newStatus}`}</p>
